...
$pers->select($_POST['sup_id']);
?>
<section class="row">
    <div class="col-md-12">
        <div class="card card-info" >
            <div class="card-header bg-light">Paiement - Fournisseur : <?php echo $pers->getNomComplet(); ?></div>
            <div>
                <div class="card-body">
                    <?php
                    ...
                    ?>
                    <form method="post" action="javascript:void(0)" id="pay_facture">
                        <div class="form-row">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="hidden" name="sup_id" id="sup_id" value="<?php echo $_POST['sup_id']; ?>">
                                <?php
                                if($pay>0)
                                    {?>
                                ...
                                if($trans->select_all_nb_op($_POST['op_id'])>0 and !empty($_POST['op_id']))
                                    {?>
                                        <a href="javascript:void(0)" class="btn btn-sm more_pay_sup"><i class="fa fa-file"></i> Paiements</a> 
                                        <?php
                                    }
                                    ?>
                                </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm tab">
                                ...
                                <tbody>
                                    <?php
                                    $datas=$op->select_sup_period($_POST['sup_id'],$_SESSION['periode']);
                                    $tot_du=0;
                                    $tot_p=0;
                                    $tot_f=0;
                                    ...
                                    foreach ($datas as $key => $value) {
                                        ...
                                        echo '<a href="javascript:void(0)" class="row_sup_pay" style="cursor:pointer" id="'.$value['op_id'].'" data-id="'.$value['party_code'].'">'.number_format($m_paid['paie']).'</a>';
                                        ...
                                        echo '</ul>';
                                        if($value['is_paid']==0)
                                        echo '<button class="btn btn-light btn-sm edit_appro" style="cursor:pointer"  data-id="'.$value['op_id'].'"><i class="fa fa-edit fa-fw" ></i></button> ';
                                        echo '</td></tr>';
