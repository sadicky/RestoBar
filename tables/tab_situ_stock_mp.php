...
<div class="white-box">
               ...
               <thead>
                                        <tr>
                                            <th>Produits</th><th>Unité</th><th>Qté</th><th>PA</th><th>PAT</th><th>PV</th><th>PVT</th><th>Bén</th>
                                        </tr>
               </thead>
                  ...
                  $pvt=0;
                            ...
                            $qt_syn=$stock->stock_syn_qt($un['prod_id'],$pos,$_SESSION['periode']);
                            ...
                            $pa =$det->getAmount()*$stock->getQuantity();
                            $pv =$un['prod_price']*$stock->getQuantity();
                            echo '<tr>
                           <td>'.$un['prod_name'].'</td><td>'.$un['unt_mes'].'</td><td>';

                              echo $pers->nb_format($stock->getQuantity());

                            echo '</td><td>'.$pers->nb_format($det->getAmount()).'</td><td>'.$pers->nb_format($det->getAmount()*$stock->getQuantity()).'</td><td>'.$pers->nb_format($un['prod_price']).'</td><td>'.$pers->nb_format($un['prod_price']*$stock->getQuantity()).'</td><td>'.$pers->nb_format($pv-$pa).'</td></tr>';
                          }
                    ...
                    <tfoot>
                      <tr>
                        <th>Total</th><th>-</th><th>-</th><th>-</th><th><?php echo $pers->nb_format($pat); ?></th><th>-</th><th><?php echo $pers->nb_format($pvt); ?></th><th><?php echo $pers->nb_format($pvt-$pat); ?></th>
                      </tr>
                    </tfoot>
               </table>
              </div>
